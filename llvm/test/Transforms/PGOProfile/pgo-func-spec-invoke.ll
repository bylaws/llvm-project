...
define i32 @compute(i32 %x) personality ptr @__gxx_personality_v0 !prof !27 {
entry:
...
if.end:
  %result = phi i32 [ %d, %if.then ], [ %j, %if.else ]
  ret i32 %result
}

declare i32 @__gxx_personality_v0(...)

define i32 @caller(i32 %arg) personality ptr @__gxx_personality_v0 !prof !27 {
entry:
  ; CHECK-LABEL: @caller
  ; CHECK: switch i32
  ;
  ; CHECK: CS.Case.0:
  ; CHECK: invoke i32 @compute.spec.0.42(i32 42)
  ; CHECK-NEXT: to label %CS.Merge unwind label %catch
  ;
  ; CHECK: CS.Default:
  ; CHECK: invoke i32 @compute(i32 %arg)
  ; CHECK-NEXT: to label %CS.Merge unwind label %catch
  ;
  ; CHECK: CS.Merge:
  ; CHECK: %CS.RVMerge = phi i32
  ; CHECK: br label %normal
  ;
  ; CHECK: normal:
  ; CHECK: ret i32 %CS.RVMerge
  %result = invoke i32 @compute(i32 %arg)
          to label %normal unwind label %catch, !prof !29
...
; CHECK: define {{.*}} @compute.spec.0.42
