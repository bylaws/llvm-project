...
static cl::opt<bool> ForceSpecialization(
    "force-pgo-specialization", cl::init(false), cl::Hidden, cl::desc(
    "Force function specialization for every call site with a constant "
    "argument"));

static cl::opt<unsigned> MinFunctionSize(
    "pgofuncspec-min-function-size", cl::init(500), cl::Hidden,
...
Function *PGOFunctionSpecializer::cloneFunctionSpecialized(Function *F, 
  ...
  LPM2.addPass(LoopFullUnrollPass(3,
                                  false,
                                  false));
...
bool PGOFunctionSpecializer::run() {
  ...
  if (!NumCandidates) {
    dbgs() << "PGOFnSpecialization: No possible specializations found in module\n";
    return false;
  }
  ...
  for (auto &[CS, Specs] : CallSiteSpecs) {
    ...
    if (auto *Invoke = dyn_cast<InvokeInst>(CS)) {
      ...
      UnwindBB = Invoke->getUnwindDest(); // FIXME fixup PHIS???
    ...
    if (ArgType->isFloatTy() || ArgType->isDoubleTy()) {
      IntType = ArgType->isFloatTy() ? Type::getInt32Ty(Ctx) : Type::getInt64Ty(Ctx);
    ...
    for (PGOSpec *SpecPtr : Specs) {
      ...
      if (IntType) {
        CaseVal = ConstantInt::get(IntType,
          cast<ConstantFP>(CaseVal)->getValue().bitcastToAPInt().getZExtValue());
      }
...
bool PGOFunctionSpecializer::findSpecializations(Function *F, unsigned FuncSize,
  ...
  for (User *U : F->users()) {
    ...
    for (auto [A, VPIdx] : Args) {
      ...
      if (BBEdgeCount) {
        // Use block profile count as the total if available, it is more accurate as value profile counts will miss rare value counts.
        TotalCount = std::max(TotalCount, *BBEdgeCount);
      }
      dbgs() << "TotalCount " << TotalCount << " , BBEdgeCount " << (BBEdgeCount ? 0 : *BBEdgeCount) << '\n';
      ...
      for (const auto &ProfiledValue : ValueProfData) {
        // Drop values that have a percentage of calls below the configured cutoff.
        uint64_t ValProp = (ProfiledValue.Count * 100) / TotalCount;
        dbgs() << "Value " << ProfiledValue.Value << " , ValProp " << ValProp << '\n';

        if (ValProp < AnalysisCutoffThresh)
          continue;


        // Convert the profiled value to a constant type
        ...
        // Create a specialization signature for this argument-constant pair.
        PGOSpecSig S;
        ...
        if (auto It = UniqueSpecs.find(S); It != UniqueSpecs.end()) {
          ...
          dbgs() << "PGOFnSpecialization: Original CodeSize=" << OriginalCodeSize
...
PreservedAnalyses PGOFunctionSpecializationPass::run(Module &M, ModuleAnalysisManager &AM) {
