...
void AArch64Arm64ECCallLowering::getThunkType(FunctionType *FT,
                                              ...
                                              FunctionType *&Arm64Ty,
                                              FunctionType *&X64Ty) {
  Out << (TT == ThunkType::Entry ? "$ientry_thunk$cdecl$" : "$iexit_thunk$cdecl$");
...
Function *AArch64Arm64ECCallLowering::buildExitThunk(FunctionType *FT,
  ...
  getThunkType(FT, Attrs, ThunkType::Exit, ExitThunkStream, Arm64Ty,
               X64Ty);
...
bool AArch64Arm64ECCallLowering::runOnModule(Module &Mod) {
  ...
  I8PtrTy = PointerType::getUnqual(M->getContext());
  ...
  if (!ThunkMapping.empty()) {
    Type *VoidPtr = PointerType::getUnqual(M->getContext());
...
bool AArch64Arm64ECCallLowering::processFunction(
  ...
  if (F.hasExternalLinkage() || F.hasAddressTaken() || F.hasWeakLinkage() || F.hasLinkOnceLinkage()) {
