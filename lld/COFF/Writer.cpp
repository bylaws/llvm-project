...
void Writer::createExportTable() {
  ...
  auto addTargetChunks = [&](COFFTargetContext &target) {
  ...
  if (!edataSec->chunks.empty()) {
    ...
    if (ctx.primaryTarget.hadExplicitExports)
    ...
    addTargetChunks(ctx.primaryTarget);
  }
  if (ctx.hybridTarget) {
    addTargetChunks(*ctx.hybridTarget);
  }
  ...
  for (OutputSection *sec : ctx.outputSections) {
    ...
    for (Chunk *c : sec->chunks) {
      ...
      if (padding && c->isHotPatchable())
        virtualSize += padding;
      if (isa<SectionChunk>(c) && c->getMachine() == ARM64EC &&
          ctx.getTarget(ARM64EC).symtab.findECEntryThunk(cast<SectionChunk>(c)))
